...
1. **대화 기록 실제 연동** — 사이드바 히스토리가 지금은 하드코딩. `GET /api/chat/sessions` 같은 조회 API를 추가하고 프론트에서 실제 DB 데이터를 불러오도록 연결
2. **설정 값 저장** — 성격/애정도/목소리 선택이 새로고침하면 날아감. 우선 localStorage로 저장하거나, 캐릭터별 사용자 설정을 백엔드에 영속화
3. **음성 모드 실제 구현** — 지금은 UI 상태(듣는 중/생각 중/말하는 중)만 있고 실제 STT/TTS 없음. 브라우저 Web Speech API로 우선 붙이거나, 서버 사이드 STT/TTS 검토
4. **에러/타임아웃 처리 보강** — Ollama가 꺼져 있거나 느릴 때의 UX(현재는 폴백 문구 하나뿐), 백엔드 요청 타임아웃 설정
5. **테스트 코드** — 백엔드 `ChatService`/`ChatController` 단위 테스트, 프론트 컴포넌트 테스트 아직 없음
6. **PWA 마무리** — "홈 화면에 추가" 배너가 지금은 UI만 있고 실제 manifest.json/서비스워커 없음
7. **배포 방식 결정** — 로컬 전용으로 둘지, 프로덕션 빌드(백엔드가 프론트 정적 파일 서빙 등)를 준비할지
